...
Page({
    ...
    onReachBottom: function() {
      ...
      setTimeout(() => {
        ...
        a.post("goods.get_list", {
          page: that.data.curpage,
        }, function (res) {
          if (res.error == 0 ) {
            if (res.list != "") {
              let arrList = that.data.lists.concat(res.list)
              that.setData({
                lists: arrList
              })
            } else {
  ...
  onGps(){
    ...
    // getMerchInfo(){
    //   let that = this;
    //   a.post("merch.list.main",{
    //     lat: that.data.latitude,
    //     lng: that.data.longitude,
    //     page:1,
    //     favorite:0
    //   },function(res){
    //     // console.log(res.list[0]+"resssssss")
    //     if (res.error == 0) {
    //       t.globalData.mdid = res.list[0].id
    //     }else{
    ...
    getmdInfo(){
      ...
      a.post("merch.list.getMerchInfo",{
        id: t.globalData.typemdid,
        type: t.globalData.mdtype,
        lat: that.data.latitude,
        lng: that.data.longitude,
      },function(res){
        console.log(res)
